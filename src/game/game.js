...
(function () {
  ...
  function spawnFood() {
    ...
    // pilih random link adsense untuk food ini
    const link = randomLink(); // bisa ganti dengan link custom lo juga

    return { x: fx, y: fy, link };
  }
  ...
  function loop() {
    ...
    loopTimer = setTimeout(() => {
      ...
      if (food && head.x === food.x && head.y === food.y) {
        ...
        // buka link food
        try { window.open(food.link, "_blank"); } catch {}

        food = spawnFood();

        // tiap 10 poin, random link tambahan
